...
$result = getSearchResult(function (Elasticsearch\Client $client)use($keyword,$page)
{
...
$smarty->assign('goods_list', resultToGoods($result));
...
$pager = get_pager('search.php', $pager['search'], $result['hits']['total'], $page, 40);
$display  = (isset($_REQUEST['display']) && in_array(trim(strtolower($_REQUEST['display'])), array('list', 'grid', 'text'))) ? trim($_REQUEST['display'])  : (isset($_SESSION['display_search']) ? $_SESSION['display_search'] : 'grid');
